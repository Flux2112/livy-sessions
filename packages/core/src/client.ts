...
export class LivyClient {
  ...
  async listBatches(signal?: AbortSignal): Promise<LivyBatch[]> {
    const res = await request<BatchListResponse>(this.opts('GET', '/batches', undefined, signal))
    return res.sessions as LivyBatch[]
  }

  async createBatch(req: CreateBatchRequest, signal?: AbortSignal): Promise<LivyBatch> {
    return request<LivyBatch>(this.opts('POST', '/batches', req, signal))
  }

  async getBatch(id: number, signal?: AbortSignal): Promise<LivyBatch> {
    return request<LivyBatch>(this.opts('GET', `/batches/${id}`, undefined, signal))
  }

  async deleteBatch(id: number, signal?: AbortSignal): Promise<void> {
    await request<void>(this.opts('DELETE', `/batches/${id}`, undefined, signal))
  }
  ...
  async getBatchLogs(
    ...
    if (size !== undefined) params.set('size', String(size))
    const query = params.toString() ? `?${params.toString()}` : ''
    return request<LogResponse>(this.opts('GET', `/batches/${batchId}/log${query}`, undefined, signal))
  }

  async getBatchState(id: number, signal?: AbortSignal): Promise<BatchStateResponse> {
    return request<BatchStateResponse>(this.opts('GET', `/batches/${id}/state`, undefined, signal))
  }
}
